...
// Tutorial slides are really static so I'm just trying to treat this as a static HTML-ish page with flexible JS syntax, and there is always
// room to add in more interactive components.

const INTRO_MSG =
  "Welcome to Pentaquad! This is a more complex variant of Tetris, with larger pieces that fall both down and right.";
...
const MOVE_CONTROLS_MSG =
  "Use the MOUSE to guide the piece. Use R to rotate, F to flip, and SPACE to drop the piece.";
...
const PALETTE_MSG =
  "The current piece can also be held for later use in an inventory slot, specified by pressing any number key between 1-5. If the slot already has a piece, that piece will become the current one.";
...
const OBJECTIVE_MSG =
  "Clearing lines both horizontally and vertically will earn points just like in Tetris, but this game revolves around filling square targets, shown below. ";
...
const TARGET_MSG =
  "Targets that are not cleared in time result in a strike, of which three end the game. If a target is cleared, it destroys bonus blocks around it and gives a power-up to the next piece. You can see flashing squares where a target will spawn shortly before it actually does.";
...
const POWERUP_MSG =
  "There are four different types of power-ups, which have different animations shown here";
...
export const SlideWrapper = (props) => {
  return <div style={{ ...container, ...slideStyle }}>{props.children}</div>;
};
...
export const PowerupSlide = () => {
  return (
    <div>
      <div>
        <div style={powerupEntryStyle}>{POWERUP_MSG}</div>
        <div style={powerupEntryStyle}>{GHOST_PIECE_MSG}</div>
        <div style={powerupEntryStyle}>{BOMB_PIECE_MSG}</div>
        <div style={powerupEntryStyle}>{DRILL_PIECE_MSG}</div>
        <div style={powerupEntryStyle}>{TOWER_PIECE_MSG}</div>
