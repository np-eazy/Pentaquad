...
const Scorekeeper = class {
    ...
    scoreTargets(targetsCleared) {
        this.score += this.targetCombo * this.lineCombo * BASE_TARGET_SCORE * targetsCleared * (targetsCleared + 1) / 2; 
        if (targetsCleared == 0) {
            this.targetCombo = 1;
        } else {
    ...
    scoreFilledLines(linesCleared) {
        this.score += this.targetCombo * this.lineCombo * BASE_LINE_SCORE * linesCleared * (linesCleared + 1) / 2; 
        if (linesCleared == 0) {
            this.lineCombo = 1;
        } else {
